...
import { ChevronRight, ChevronLeft, Loader2, Check, AlertCircle, Lock, Image as ImageIcon, RefreshCw, Save, XCircle } from "lucide-react"
...
async function ocrINE(file: File): Promise<string> {
  ...
  const r = await fetch("https://api.ocr.space/parse/image", { method: "POST", headers: { apikey: OCR_KEY }, body: fd })
  const d = await r.json()
...
function extractNameFromINE(text: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    const l = lines[i]
    if (!nombres && /[MN][OO0][MN]?B[RB]E/.test(l)) nombres = valAfter(/[MN][OO0][MN]?B[RB]E[S()#]*\s*/g, lines, i)
    else if (!ap && /APELLIDO\s*(PATERNO|PAT)|PRIMER\s*APELLIDO/.test(l)) ap = valAfter(/APELLIDO\s*(PATERNO|PAT\.?)|PRIMER\s*APELLIDO\s*/g, lines, i)
    else if (!am && /APELLIDO\s*(MATERNO|MAT)|SEGUNDO\s*APELLIDO/.test(l)) am = valAfter(/APELLIDO\s*(MATERNO|MAT\.?)|SEGUNDO\s*APELLIDO\s*/g, lines, i)
  }
...
export default function DriverSetup({ userId, userEmail, initialData, onSubmit, onCancel }: Props) {
  ...
  const handleIneFrente = async (file: File) => {
    ...
    try {
      ...
      if (extracted && extracted.trim().length >= 3) { setNombre(extracted.trim()); setOcrFailed(false) }
      else { setOcrFailed(true); setIneFrente(null); setIneFrentePreview("") }
    } catch { setOcrFailed(true); setIneFrente(null); setIneFrentePreview("") }
    finally { setOcrLoading(false) }
  }

  const handleSubmit = async () => {
    if (!ineFrente || !ineReverso || !tarjetaCirc) { setError("Completa todos los campos."); return }
    setLoading(true); setError("")
    try {
      const [fUrl, rUrl, tUrl] = await Promise.all([uploadCloudinary(ineFrente), uploadCloudinary(ineReverso), uploadCloudinary(tarjetaCirc!)])
      await enviarSolicitudRepartidor({ userId, email: userEmail, nombre, telefono, vehiculo_tipo: vehiculoTipo, vehiculo_modelo: vehiculoModelo, vehiculo_placas: vehiculoPlacas, ine_frente_url: fUrl, ine_reverso_url: rUrl, selfie_url: "", tarjeta_url: tUrl })
      clearSetup(); onSubmit()
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 0 && (
            <motion.div key="s0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
              <div>
                ...
                <p className="text-sm text-slate-500">Sube el frente de tu INE. Leemos tu nombre automaticamente.</p>
              </div>
              {ocrFailed && (
                <div className="mt-4 p-4 bg-yellow-50 border border-yellow-300 rounded-xl">
                  <p className="text-sm text-yellow-800 mb-2 font-medium">No se pudo leer la INE. Escribe tu nombre manualmente:</p>
                  <input type="text" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400"
                    placeholder="Nombre completo segun tu INE" value={nombreManual ?? ""}
                    onChange={(e) => setNombreManual(e.target.value)} />
                  <button onClick={() => { if (nombreManual?.trim()) { setNombre(nombreManual.trim()); setOcrFailed(false) } }}
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-red-50 border-2 border-red-200 rounded-3xl p-6 text-center space-y-4">
                  ...
                  <div>
                    ...
                    <ul className="text-red-500 text-xs mt-3 space-y-1 text-left">
                      <li>Buena iluminacion, sin sombras ni reflejos</li>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-black text-slate-500 uppercase tracking-wider">Nombre (de tu INE)</label>
                      {nombreLocked && (<div className="flex items-center gap-1 text-green-600 text-xs font-black"><Lock size={12} /> Confirmado</div>)}
                    </div>
                    <input value={nombre} onChange={e => !nombreLocked && setNombre(e.target.value.toUpperCase())} readOnly={nombreLocked}
                      className="w-full px-4 py-3.5 rounded-2xl text-sm font-black uppercase outline-none transition-all border-2"
                      style={{ background: nombreLocked ? "#f0fdf4" : "#f8fafc", borderColor: nombreLocked ? "#86efac" : "#e2e8f0", color: "#1e293b" }} />
                  </div>
                  {!nombreLocked && (
                    <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3 flex gap-2">
                      <AlertCircle className="text-amber-500 shrink-0 mt-0.5" size={16} />
                      <p className="text-amber-700 text-xs font-bold leading-relaxed">Verifica que sea exactamente como aparece en tu INE. Si no coincide, tu solicitud sera rechazada.</p>
                    </div>
                  )}
                  {!nombreLocked ? (
                    ...
                    <p className="text-xs text-green-600 font-bold text-center">Este nombre se usara en todos tus pedidos</p>
                  )}
                </motion.div>
              )}
          ...
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
              ...
              <div className="flex gap-4">
                <button onClick={() => saveStep(0)} className="flex-1 py-4 bg-slate-100 text-slate-600 font-bold rounded-2xl flex items-center justify-center gap-2"><ChevronLeft size={20} /> Atras</button>
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
              <div><h1 className="text-2xl font-black text-slate-900 mb-1">Tu Vehiculo</h1><p className="text-sm text-slate-500">Informacion de transporte y contacto.</p></div>
              ...
              <div className="space-y-3">
                {[{ value: vehiculoModelo, set: setVehiculoModelo, label: "Modelo", placeholder: "Ej: Italika FT150, Tsuru...", upper: false },
                  ...
                  { value: telefono, set: setTelefono, label: "Telefono de contacto", placeholder: "55 1234 5678", upper: false }
                ].map(({ value, set, label, placeholder, upper }) => (
                  <div key={label} className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{label}</label>
                    <input value={value} onChange={e => { const v = upper ? e.target.value.toUpperCase() : e.target.value; set(v); if(label==="Telefono de contacto") saveTel(v); if(label==="Modelo") saveVModelo(v); if(label==="Placas") saveVPlacas(e.target.value.toUpperCase()) }} placeholder={placeholder}
              ...
              <div className="flex gap-4">
                <button onClick={() => saveStep(1)} className="flex-1 py-4 bg-slate-100 text-slate-600 font-bold rounded-2xl flex items-center justify-center gap-2"><ChevronLeft size={20} /> Atras</button>
                <button onClick={() => saveStep(3)} disabled={!vehiculoModelo || (vehiculoTipo !== "bici" && !vehiculoPlacas) || !telefono}
            ...
            <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-3">
                ...
                <p className="text-amber-700 text-xs font-bold leading-relaxed">
                  {vehiculoTipo === "bici" ? "La foto debe mostrar claramente tu bicicleta completa." : "Las placas y datos del vehiculo deben coincidir exactamente con lo que ingresaste. De lo contrario tu solicitud sera rechazada."}
                </p>
              </div>
              <div className={`border-2 border-dashed rounded-3xl overflow-hidden transition-all ${tarjetaCirc ? "border-green-400" : "border-slate-200"}`}>
                {tarjetaCirc ? (
                  <div className="relative">
                    ...
                    <div className="absolute top-2 right-2 bg-green-500 text-white p-1.5 rounded-full"><Check size={16} /></div>
                    <button onClick={() => setTarjetaCirc(null)} className="absolute top-2 left-2 bg-white/90 rounded-full p-1.5 shadow"><RefreshCw size={16} className="text-slate-600" /></button>
                  </div>
                ) : (
                  <div className="p-8 text-center space-y-4">
                    <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center mx-auto">
                      <span className="text-3xl">{vehiculoTipo === "bici" ? "B" : "A"}</span>
                    </div>
                    <div>
                      <p className="font-bold text-slate-700">{vehiculoTipo === "bici" ? "Foto bicicleta" : "Tarjeta de Circulacion"}</p>
                      <p className="text-xs text-slate-400 mt-1">Asegurate que las placas sean legibles</p>
                    </div>
                    <div className="flex gap-2 justify-center">
                      <label className="px-4 py-2 bg-slate-100 rounded-xl cursor-pointer text-sm font-bold text-slate-600 hover:bg-slate-200 transition-all">
              ...
              <div className="flex gap-4">
                <button onClick={() => saveStep(2)} className="flex-1 py-4 bg-slate-100 text-slate-600 font-bold rounded-2xl flex items-center justify-center gap-2"><ChevronLeft size={20} /> Atras</button>
                <button onClick={() => saveStep(4)} disabled={vehiculoTipo !== "bici" && !tarjetaCirc}
                  className="flex-[2] py-4 bg-slate-900 text-white font-bold rounded-2xl flex items-center justify-center gap-2 disabled:opacity-50">Siguiente <ChevronRight size={20} /></button>
              </div>
            </motion.div>
          )}
          ...
          {step === 4 && (
            <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
              ...
              <div className="bg-slate-50 rounded-3xl p-5 space-y-3">
                ...
                {[["Telefono", telefono], ["Vehiculo", `${vehiculoTipo.charAt(0).toUpperCase() + vehiculoTipo.slice(1)} — ${vehiculoModelo}`], ...(vehiculoTipo !== "bici" ? [["Placas", vehiculoPlacas]] : []), ["Documentos", "INE frente + reverso + " + (vehiculoTipo === "bici" ? "foto bici" : "tarjeta circulacion")]].map(([k, v]) => (
              ...
              <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100 flex gap-3">
                ...
                <p className="text-blue-700 text-xs font-bold leading-relaxed">Tu solicitud sera revisada en 24-48 horas.</p>
              </div>
