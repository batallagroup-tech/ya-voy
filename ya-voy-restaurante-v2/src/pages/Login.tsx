...
const GRAD = 'linear-gradient(135deg, #FF6B00 0%, #E65F00 50%, #cc4400 100%)';
const ACCENT = '#FF6B00';
          ...
          <motion.div key="sel" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="w-full flex flex-col gap-3 mt-4">
              <button onClick={handleGoogle} disabled={loading}
                className="w-full bg-white text-orange-600 font-bold py-4 rounded-2xl flex items-center justify-center gap-3 shadow-xl hover:bg-orange-50 transition-all disabled:opacity-60">
          ...
          <motion.div key="email" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}
            ...
            <form onSubmit={handleEmail} className="flex flex-col gap-4">
              ...
              <button type="submit" disabled={loading}
                className="w-full bg-white text-orange-600 font-black py-4 rounded-2xl flex items-center justify-center gap-2 shadow-xl hover:bg-orange-50 transition-all disabled:opacity-60 mt-2">
                {loading && <Loader2 className="animate-spin" size={20} />}
                Entrar
              </button>
            ...
            {resetSent ? (
              <div className="flex flex-col gap-4">
                ...
                <button onClick={() => go('selection')}
                  className="w-full bg-white text-orange-600 font-black py-4 rounded-2xl flex items-center justify-center shadow-xl hover:bg-orange-50 transition-all">
                  Volver al inicio
                </button>
              </div>
            ) : (
              <>
                ...
                <form onSubmit={handleRequestReset} className="flex flex-col gap-4">
                  ...
                  <button type="submit" disabled={loading || !email}
                    className="w-full bg-white text-orange-600 font-black py-4 rounded-2xl flex items-center justify-center gap-2 shadow-xl hover:bg-orange-50 transition-all disabled:opacity-60">
                    {loading && <Loader2 className="animate-spin" size={20} />}
                    Enviar link
                  </button>
                </form>
